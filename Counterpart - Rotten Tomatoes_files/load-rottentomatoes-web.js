...
//[v3.57.1] patch for todayshow
if(typeof mpscall === 'object' && mpscall.site && mpscall.site.indexOf("todayshow") > -1){
  mps._ext.nowrite="jq";
}
...
mps._gptloadCallback = function(){
  ...
  mps._debug('[mps:QUEUE] (processing queue items) gptloaded '+mps._elapsed());
};
...
mps._execAd = function(slot,loadset,clone,detect) {
  try {
    ...
    if(mps.lazyload) {
      var _lazyloadslot = mps.lazyload[loadset] && mps.lazyload[loadset].adslots && mps.lazyload[loadset].adslots.indexOf(slot) > -1;
      var _detectelem = (loadset !== 0 || (mps._detect && typeof mps._detect.detectionelement[slot] !== 'boolean') || (mps._detect && mps._detect.detectionelement[slot] === true));
      if(!clone && slot !== '_oop' && _lazyloadslot && (!_detectelem || !mps._detect.viewport(document.getElementById(gptdiv))) && !detect) {
        var detectAd = { 'slot': slotvar, 'loadset': loadset };
        mps._detect._pending.push(detectAd);
        return false;
      }
      // For Clones: lazyload initial, parent removed or cloneslot
      if(clone > 0 && !detect && (_lazyloadslot || (mps.lazyloadclone[loadset] && mps.lazyloadclone[loadset].indexOf(slotvar) > -1)) && (!_detectelem || !mps._detect.viewport(document.getElementById(gptdiv)))) {
        var detectAd = { 'slot': slotvar, 'loadset': loadset };
        mps._detect._pending.push(detectAd);
        mps._clonerunning = false;
        return false;
    ...
    if(mps._ext.dart_mode === 'gpt-asynchronous') {
      mps._gptcmd.push(function () {
        ...
        if (!mps.slotsdisabled[loadset] || (mps.slotsdisabled[loadset] && mps.slotsdisabled[loadset].indexOf(slotvar) === -1)) {
          var _gptdisplay = setTimeout(function () {
...
mps._defineAdSlot = function(mpsid, slotobj) {
  ...
  for (var k in _slotobj.targeting) {
    if(_slotobj.targeting.hasOwnProperty(k) && mps.response.dart.params.allowable_targeting_keys.length && mps.response.dart.params.allowable_targeting_keys.indexOf(k) > -1) {
...
mps._execgptinit = function() {
  ...
  if (typeof(mps.response.dart.params.gpt_init_js) === 'string' && mps.response.dart.params.gpt_init_js.length > 0) {
    ...
    try {
      if (mps._bool(mps._jqenabled) === true) {
        $('head').append(_gptinit);
      } else {
...
mps._execgpt = function() {
  ...
  var _mpsid = mps.response.dart.params.mpsid;
  var _prefix = mps.response.dart.params.ad_div_prefix;
  var _adslots = mps.response.dart.params.adunits;
  var _path = mps.response.dart.params.path;
      ...
      if (i !== '_oop' && mps._keys(_adslot.responsive_map).length > 0 && mps._keys(_responsiveset).length > 0) {
        ...
        for (var k in _adslot.responsive_map) {
          if (_adslot.responsive_map[k].indexOf('NONE') === -1) {
            ...
            if(mps.slotsdisabled[_loadset] && mps.slotsdisabled[_loadset].indexOf(k) === -1) {
  ...
  // Disable Inital Load
  if(mps.response.dart.params.disable_initial_load === 1) {
    if(mps._bool(mpsopts.skipdisableinitialload) === false) {
      googletag.pubads().disableInitialLoad();
    }
  }

  // Enable Single-Request Mode
  if(mps.response.dart.params.enable_single_request_mode === 1){
...
mps.getAd = function(adunit, _swap, loadset) {
  try {
    var _adunit = adunit;
    var _slotid = 'mps-getad-' + _adunit.toLowerCase().replace(/\s+/g, '');
    ...
    if (typeof(adunit) != 'undefined' && typeof(mps) === 'object' && typeof(mps.response) === 'object' && typeof(mps.response.dart) === 'object' && typeof(mps.response.dart.adunits) === 'object' && typeof(mps.response.dart.adunits[adunit]) === 'object' && typeof(mps.response.dart.adunits[adunit].data) !== 'undefined') {
      ...
      if (mps.response.dart.adunits[adunit].data !== '') {
        ...
        mps._debug("^[mps:preLoader] mps.getAd(" + _adunit + ") SKIPPED: Disabled " + mps._elapsed());
        adslothtml = '<!--(mps.getAd) ' + adunit + ' disabled-->';
      }
    } else {
      if (_swap) {
        mps._debug("^[mps:preLoader] mps.getAd('" + _adunit + "') QUEUED: Pending GPT " + mps._elapsed());
...
mps._xhr = function() {
  ...
  mpsxhr.onload = function(data) {
    if (mpsxhr.status === 200) {
      mps.response = JSON.parse(data.currentTarget.response);
      // MPS Callback.
      mpsCallback(mps.response);

      // Lazyload
      if(typeof mps._detect === 'object' && mps.response && mps.response.dart && mps.response.dart.params && mps.response.dart.params.lazy_load_slots && mps.response.dart.params.lazy_load_slots.length) {
        mps.lazyload = mps.lazyload || {};
        mps.lazyload[mps._loadset || 0] = mps.lazyload[mps._loadset || 0] || mps.response.dart.params.lazy_load_slots;
      }
      // Define adslots with gpt, once gpt.js is loaded.
      var _mpssyncdelay = mps._ext.dart_mode === 'gpt-asynchronous'? 0 : 500;

      mps._gptcmd.push(function() {
        setTimeout( function() { mps._execgptinit(); }, _mpssyncdelay );
      });
      mps._gptcmd.push(function() {
        setTimeout( function() { mps._execgpt(); }, _mpssyncdelay );
      });
...
/* [2017-02-16] <START> Header Bidding (v4) */ mps.hb=mps.hb||{}; mps.hb.executed=0;
/*====[config]============================*/
mps.hb.a9pubid = '';
mps.hb.ixjs = '185796-219213027941318.js';
/*========================================*/
// @TODO !c=headerbid
...
if(typeof(mpscall.title)=='undefined' && typeof(document.title)=='string' && document.title.length > 0) mpscall.title = mps._trim(document.title.replace(/[^-a-z0-9\s\[\]._():]/ig,'')).replace(' - Rotten Tomatoes','').substring(0,250);
...
var tm = (mps._ext.tm_redirect === 1) ? ("&tm="+Math.floor(Math.random() * 9999) + 1) : "";
mps.requesturl = mps._ext.mpsRequestUrl() + tm;
  ...
  mps._kvinsert = {
    ...
    require: function(type) {
      if(!type || type.length==0){
        mps._debug('^[mps:kvinsert] invalid require()');
        return false;
      }
      ...
      setTimeout(function () {
        if (mps._kvinsert.ready.indexOf(type) == -1) {
          mps._debug('^[mps/kvinsert] Force complete: '+type+' after '+mps._delays.kvtimeout+'ms');
    ...
    complete: function(type,force){
      if(!type || type.length===0){
        mps._debug('^[mps:kvinsert] invalid complete()');
        return false;
      }
      force = force || false;
      mps._reqs = mps._reqs || {};
...
mps.execute_lazyload_js = function() {
  ...
  mps._detect.elementsListener = function() {
    for(var i in mpsopts.detectionelement) {
      if(!mps._detect.detectionelement[i] && mps._select(mpsopts.detectionelement[i]) && mps._select(mpsopts.detectionelement[i]).clientHeight > 0) {
        mps._debug('[mps:Detect]: Detect element ' + mpsopts.detectionelement[i] + ' is in viewport and has a height greater than 0.');
  ...
  mps._detect.elements = function() {
    ...
    if(!mps._detect._timeout) {
      ...
      mps._debug('[mps:Detect]: Detect element timeout reached.');
      clearInterval(mps._detect.parentint);
    }
  };
  
  //--mps._detect.viewport(s[element])
  mps._detect.viewport = function(s) {
    if(typeof s === 'object' && s !== null && (!mps._bool(mpsopts.activeload) || (mps._bool(mpsopts.activeload) === true && mps._activerefresh._focus === true))) {
      var _dataset = s.dataset;
      var _slotname = _dataset && _dataset.mpsSlot ? _dataset.mpsSlot : null;
      var _loadset = _dataset && _dataset.mpsLoadset ? parseInt(_dataset.mpsLoadset) : null;
      if(_loadset === 0 && _slotname && typeof(mpsopts.detectiondelay) === 'object' && mpsopts.detectiondelay[_slotname]) {
        if (parseInt(mpsopts.detectiondelay[_slotname]) > (Date.now() - mps._timer)) {
          mps._debug('[mps:Detect] delay detected display for ' + _slotname + ':' + mpsopts.detectiondelay[_slotname] +'ms.');
          return false;
        }
      }
      var win = window, winDocElem = win.document.documentElement, docHeight = mps._getDocHeight(), vp = mps._viewport(), vpW = vp[0], vpH = vp[1], elO = mps._getBoundingRect(s), elY = elO.top, elX = elO.left,
      ...
      //TODO only display msg once
      // mps._debug(mps._bool(mpsopts.activeload) === true ? '[mps:Detect] Activeload - window not focused. ' + s : '[mps:Detect] Invalid object: ' + s);
      return false;
    }
  };
  
  //--mps._detect.load loads or refreshes ad
  mps._detect.load = function() {
    var ads = mps._detect._pending;
    if(ads.length > 0) {
      for(var i =0; i<ads.length; i++) {
        var _ads = ads[i], gptdiv = mps._adslots[_ads.loadset][_ads.slot];
        if(typeof document.getElementById(gptdiv) == 'object' && ((_ads.loadset !== 0 || typeof(mps._detect.detectionelement[_ads.slot]) !== 'boolean') || mps._detect.detectionelement[_ads.slot] === true) && mps._detect.viewport(document.getElementById(gptdiv))) {
    ...
    for(var i in refresh) {
      for(var j in refresh[i]) {
        if(((i !== 0 || typeof(mps._detect.detectionelement[refresh[i][j]]) !== 'boolean') || mps._detect.detectionelement[refresh[i][j]] === true) && mps._detect.viewport(document.getElementById(refresh[i][j])) && (mpsopts.activerefresh != 2 || (mpsopts.activerefresh == 2 && mps._activerefresh._focus === true))) {
  ...
  //--mps._detect.remove(slot[string], loadset[number])
  ...
  //--mps._detect.forceLoad(slot[string], loadset[number]) force loads from _pending
  mps._detect.forceLoad = function(slot, loadset) {
    // If loadset isn't passed, default to current loadset.
    ...
    for (var i = 0; i < mps._detect._pending.length; i++) {
      if (mps._detect._pending[i].slot === slot && mps._detect._pending[i].loadset === loadset) {
        ...
        mps._debug('[mps:Detect]: Force loading ' + slot
          + (clone ? ('(clone ' + clone + ')') : '')
          + ' in loadset ' + loadset + ', call mps._execAd(), remove from _pending.');
        if (mps._execAd(slot, loadset, clone, 1)) {
          mps._detect.remove(slot, loadset);
        }
      }
    }
    if (mps._detect._refresh[loadset] && mps._detect._refresh[loadset][slot]) {
      _slotfound = true;
      mps._debug('[mps:Detect]: Force loading ' + slot + ' in loadset ' + loadset + ', call mps.refreshAds(), remove from _refresh.');
    ...
    if (!_slotfound) {
      mps._debug('[mps:Detect]: slot ' + slot + ' not found in loadset ' + loadset);
    }
    return _slotfound;
  };
  ...
  if(window.addEventListener) {
    mps._scroll = window.addEventListener('scroll', mps._detect.load, {passive:true}); mps._resize = window.addEventListener('resize', mps._detect.load);
  } else {
    mps._scroll = window.attachEvent('onresize', mps._detect.load, {passive:true}); mps._resize = window.attachEvent('onscroll', mps._detect.load);
  }
...
mps.execute_refresh_js = function() {
  ...
  mps.refreshDetect = function(slotname, loadset, _skipresponsive) {
    ...
    if((typeof loadset != 'undefined' || typeof loadset === 'number') && typeof slotname != 'object') {
      ...
      lazyLoadEnabled = !!(!_skipresponsive &&  (mps.lazyload[loadset].adslots.indexOf(cleanslot) > -1) || (isclone && mps.lazyloadclone[loadset] && mps.lazyloadclone[loadset].indexOf(slotname) > -1));
      // object slotname
    } else {
      slotElemId = slotname.getSlotElementId();
      slotElem = document.getElementById(slotElemId);
      slotElemName = slotElem.dataset?slotElem.dataset.mpsSlot:slotElem.getAttribute('data-mps-slot');
      if(!slotElem) {
        return false;
      }
      ls = slotElem.dataset?parseInt(slotElem.dataset.mpsLoadset):parseInt(slotElem.getAttribute('data-mps-loadset'));
      isclone = slotElemName.indexOf('-C') > -1;
      lazyLoadEnabled = !!(_skipresponsive && mps.lazyload && mps.lazyload[ls] && mps.lazyload[ls].adslots.indexOf(slotElemName) > -1) || (isclone && mps.lazyloadclone[ls] && mps.lazyloadclone[ls].indexOf(slotElemName) > -1);
    ...
    if((lazyLoadEnabled || mpsopts.activerefresh == 2) && slotElem != null && !mps._detect.viewport(slotElem)) {
        ...
        for(lsk in mps._slotscalled) {
          ...
          for(ask in mps._slotscalled[lsk]) {
            ...
            if(!mps._slotsdisabled[lsk][ask] && mps._advars[lsk][ask] && gpt[mps._advars[lsk][ask]]) {
              ...
              if((!mps.lazyload || (typeof(mps.lazyload[lsk]) === 'undefined')) && mpsopts.activerefresh != 2) {
                ...
                if(ask !== '_oop' &&  mpsopts.activerefresh == 2 || (mps.lazyload[lsk] && mps.lazyload[lsk].adslots.indexOf(askclean) > -1) || (mps.lazyloadclone && mps.lazyloadclone[lsk] && mps.lazyloadclone[lsk].indexOf(ask) > -1)) {
                  var _slotElem = document.getElementById(mps._slotscalled[lsk][ask]);
                  if(mps._detect.viewport(_slotElem) && (mpsopts.activerefresh != 2 || (mpsopts.activerefresh == 2 && mps._activerefresh._focus === true))) {
                    _slotvars[lsk].push(gpt[mps._advars[lsk][ask]]);
                  } else {
  ...
  mps.refreshAdsInterval = function (intervalsecs,slotname,loadset,intervalkey) {
    ...
    if(intervalsecs > 0) {
      ...
      mps.refreshintervals[intervalkey] = setInterval(function(){
        ...
        if(mps._bool(mpsopts.activerefresh) === true || mpsopts.activerefresh == 2) {
...
mps.execute_destroy_js = function() {
  ...
  mps._destroy.executeDestroyAdsCallback = function(slotstodestroy, loadset) {
    ...
    // #2 Single delete
    if (mps._keys(slotstodestroy).length == 1 && !destroyall){
    ...
    if(slot && typeof mps._detect === "object" && mps._keys(mps._detect._pending).length) {
      for(var i in mps._detect._pending) {
        ...
        if (slotname !== slot){
          ...
          // 1. Remove element
          var elemid = gpt[mps._advars[loadset][slotname]].getSlotElementId();
          mps._destroy.removeElement(elemid);
  ...
  mps._destroy.destroyAdsPassThrough = function(slotstodestroy, loadset){
    if(googletag.destroySlots(slotstodestroy)) {
...
/* js/internal/authorized.js */

mps.execute_authorized_js = function() {
  mps=mps||{},debugmode=debugmode||{};
  debugmode.log&&window.console||(debugmode.log=0);
  mps._elapsed||(mps._elapsed=function(){return"";});
  
  mps._log("****** YOU ARE LOGGED INTO MPS ******");
  
  mps._debug('[mps/JS] LOADED: Authorized');
}
...
mps.execute_lav_js = function() {
  ...
  mps._adlock.execute = function() {
    ...
    mps._adlock.enable = function() {
      ...
      return typeof(mps._detect) === 'object' && mps._bool(mps._adlock.opts.enable) === true && ((!mps._adlock._getResponsiveSet() || !mps._adlock.opts.breakpoints.length) ||  _breakpointsExist()) ? true : false;
    };
    if(mps._adlock.enable() === true) {
      ...
      mps._adlock.buildstyles = function () {
        var _css = '.mps-lav-fixed { position:fixed !important; }';
    ...
    mps._ready(function() {
      mps._debug('[mps/JS] LOADED: Locked Ad View');
      mps._adlock.viewport = mps._adlock._getResponsiveSet();
        ...
        mps._adlock.scroll = function () {
          ...
          for (var i in mps._adlock.loaded) {
            ...
            if (mps._adlock.loaded[i].fixed === false && (!mps._detect.viewport(_elem) || (mps._adlock.loaded[i].defaultTop - mps._adlock.displaypadding < _scrollTop))) {
              ...
              var _elemStyles = 'position:fixed !important;z-index:' + mps._adlock.opts.zindex + ';top: ' + (mps._adlock.loaded[i].defaultTop || 0) + 'px;left: ' + _elemleft + 'px;width: ' + _elem.offsetWidth + 'px;height:' + _elem.offsetHeight + 'px;';
      ...
      mps._debug("[mps:LAV] Locked ad view not enabled, verify Detected Display is enabled and correct breakpoints (mps._adlock.opts.breakpoints) are set.");
    }
  };
